    ...
    #  word char embedding ----------------------------------------
        ...
        while 1:
            ...
            for batch_index, (batch_start, batch_end) in enumerate(batches):
                ...
                # word_batch_contents = np.concatenate((word_batch_contents,char_batch_contents),axis=1)
                # batch_contents = preprocessfunc(batch_contents, keep=keep)
                word_batch_labels = labels[0][batch_ids]
                char_batch_labels = labels[1][batch_ids]

                batch_labels = labels[0][batch_ids]
                # word_batch_labels = np.concatenate((word_batch_labels, char_batch_labels))
...
def word_char_cnn_preprocess(contents , word_maxlen=Config.word_seq_maxlen, char_maxlen=Config.char_seq_maxlen, keep=False):
    ...
    word_seq = get_word_seq(contents[0], word_maxlen=word_maxlen, keep=keep)
    char_seq = get_char_seq(contents[1], char_maxlen=char_maxlen, keep=keep)
