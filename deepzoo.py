...
def get_rnn_att(maxlen, embed_weight, max_features):
    ...
    bigru = Bidirectional(GRU(units=50,activation='selu',kernel_initializer='orthogonal', recurrent_initializer='orthogonal',
...
def get_rcnn(maxlen, embed_weight,max_features):
    ...
    conv = Conv1D(128, kernel_size=3, padding='valid', kernel_initializer='glorot_uniform',kernel_regularizer=regularizers.l2(0.01))(bigru)
    avg_pool = GlobalAveragePooling1D()(conv)
    max_pool = GlobalMaxPooling1D()(conv)
    conc = concatenate([avg_pool, max_pool])
    dropfeat = Dropout(0.5)(conc)
...
def get_cgru(maxlen, embed_weight,max_features):
    ...
    conv = Conv1D(filters=128, kernel_size=3, activation='relu', padding='valid',kernel_regularizer=regularizers.l2(0.01))(embed_layer)
    bigru = Bidirectional(GRU(128, kernel_regularizer=regularizers.l2(0.01), recurrent_regularizer=regularizers.l2(0.01),return_sequences=True))(conv)
    ...
    output = Dense(19, activation='sigmoid')(fc)
    model = Model(inputs=input, outputs=output)
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    model.summary()
    return model
